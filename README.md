...
> The site deploys and works fine without this step. Push notifications need a **KV namespace** and a **VAPID secret**, configured purely in the dashboard. There is **no `wrangler.toml`** in the project, so dashboard bindings are never locked to a file.
